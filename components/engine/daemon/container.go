...
func (daemon *Daemon) newContainer(name string, operatingSystem string, config *containertypes.Config, hostConfig *containertypes.HostConfig, imgID image.ID, managed bool) (*container.Container, error) {
	...
	id, name, err = daemon.generateIDAndName(name)			//为容器生成name和id
	if err != nil {
		return nil, err
	}
	...
	if hostConfig.NetworkMode.IsHost() {
		if config.Hostname == "" {
			config.Hostname, err = os.Hostname()			//如果network是host mode，而且配置中没有指定hostname,则使用宿主机的hostname
			if err != nil {
	...
	base := daemon.newBaseContainer(id)						//生成一个最基础的container对象
	base.Created = time.Now().UTC()							//对container一些属性进行初始化
	...
	return base, err										//将生成的container对象返回
}
