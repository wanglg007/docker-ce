...
func (daemon *Daemon) findAndAttachNetwork(container *container.Container, idOrName string, epConfig *networktypes.EndpointSettings) (libnetwork.Network, *networktypes.NetworkingConfig, error) {
	...
	for {
		...
		if daemon.clusterProvider != nil {				//对cluster网络的处理
			var err error
...
func (daemon *Daemon) allocateNetwork(container *container.Container) error {
	start := time.Now()
	controller := daemon.netController				//取得daemon中的netController
	...
	defaultNetName := runconfig.DefaultDaemonNetworkMode().NetworkName()			//返回daemon使用的默认网络栈
	if nConf, ok := container.NetworkSettings.Networks[defaultNetName]; ok {
		cleanOperationalData(nConf)													//重置操作数据的endpoint设置
	...
	// If the container is not to be connected to any network,		//如果container没有连接到任何网络
	// create its network sandbox now if not present				//现在就建立它的sandbox，对于别的情况应该是在connectToNetwork()函数中创建的
	if len(networks) == 0 {
		if nil == daemon.getNetworkSandbox(container) {				//如果container还没有sandbox
			options, err := daemon.buildSandboxOptions(container)	//为sandbox创建配置信息
			if err != nil {
				return err
			}
			sb, err := daemon.netController.NewSandbox(container.ID, options...)	//为container新建一个sandbox
			if err != nil {
				return err
			}
			container.UpdateSandboxNetworkSettings(sb)				//更新sandbox的ID和key
			defer func() {
				if err != nil {
					sb.Delete()
				}
...
//主要功能为:1.查找所要connect的network，将network信息更新到container.NetworkSettings.Networks[]中;2.获取container的sandbox;3.由network创建endpoint;
...
func (daemon *Daemon) connectToNetwork(container *container.Container, idOrName string, endpointConfig *networktypes.EndpointSettings, updateSettings bool) (err error) {
	...
	sb := daemon.getNetworkSandbox(container)			//获得容器的sandbox,从controller的Sandbox数组中获取
	//创建endpoint的配置信息
	...
	ep, err := n.CreateEndpoint(endpointName, createOptions...)		//创建endpoint
	if err != nil {
		return err
	}
	defer func() {
