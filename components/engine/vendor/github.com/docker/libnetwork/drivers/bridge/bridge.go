...
func (d *driver) CreateNetwork(id string, option map[string]interface{}, nInfo driverapi.NetworkInfo, ipV4Data, ipV6Data []driverapi.IPAMData) error {
	...
	if _, ok := d.networks[id]; ok {
		d.Unlock()
		return types.ForbiddenErrorf("network %s exists", id)			//这个network id已经存在
	}
	d.Unlock()
...
//该函数作用:1.根据所要新建的network的config，验证是否与该driver的其他已经存在的network的config冲突;2.新建bridgeNetwork结构体并存储到driver的networks数组中;
//为driver的nlh赋值ns.NlHandle()，类型为*netlink.Handle;4.创建bridgeIface，并赋值给network结构体;5.再次冲突性验证;6.设置network的隔离;7.设置network的setup
//config，为network的启动做准备，其中包括setupDevice,它创建一个link
func (d *driver) createNetwork(config *networkConfiguration) error {
	var err error

	defer osl.InitOSContext()()

	networkList := d.getNetworks()						//把d.networks转成slice
	...
	return bridgeSetup.apply()				//运行上面所有的setupsteps
}
