...
// 这部分工作包括:(1)生成并初始化controller结构体;(2)初始化stores;(3)生成drvregistry,用于存储网络驱动和IPAM驱动;(4)将bridge、host、
// macvlan、null、remote、overlay等驱动初始化函数注册到drvregistry中;(5)初始化IPAM驱动注册到drvregistry中;(6)populate该controller中
// 的所有network;(7)/run/docker/libnetwork/container ID.sock开始监听。
...
// 该函数作用为:(1)对network id、network name进行有效性验证;(2)创建并初始化network结构体，对network结构体进行options处理;(3)解析networkTpye，并
// 获取该networkTpye的capability,并根据capability进行一些错误检查;(4)确保network驱动可用，如果不可用则装载;(5)为network分配ipam;(6)将network添加
// 到controller中;(7)设置network的endpoint计数;
...
func (c *controller) NewNetwork(networkType, name string, id string, options ...NetworkOption) (Network, error) {
	...
	// Construct the network object   //创建network结构体
	network := &network{
		name:        name,
	...
	if _, err := network.driver(true); err != nil {			//在分配各种资源前，确保这种类型的网络驱动是可用的
		return nil, err
	}
...
addToStore:
	...
	network.epCnt = epCnt									//network的endpoint count
	if err = c.updateToStore(network); err != nil {			//将network更新存储到store中
		return nil, err
	}
	defer func() {
		if err != nil {
...
func (c *controller) addNetwork(n *network) error {
	d, err := n.driver(true)			//获得对应network的driver，如果不存在则载入
	if err != nil {
		return err
	}
...
func (c *controller) NewSandbox(containerID string, options ...SandboxOption) (Sandbox, error) {
	...
	var sb *sandbox
	c.Lock()
	for _, s := range c.sandboxes {			//对于container已经有sandbox的处理
	...
	if sb == nil {	//首先创建sandbox和process options,Key的生成由option决定
		sb = &sandbox{
			id:                 sandboxID,
