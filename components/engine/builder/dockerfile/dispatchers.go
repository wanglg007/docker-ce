...
func dispatchRun(d dispatchRequest, c *instructions.RunCommand) error {
	...
	stateRunConfig := d.state.runConfig						//将最底层的Config结构体传入
	//容器开始要执行的命令
	cmdFromArgs := resolveCmdLine(c.ShellDependantCmdLine, stateRunConfig, d.state.operatingSystem)
	//FilterAllowed返回所有允许的args，不带过滤的args。而Env是要在容器中设置的环境变量列表
	buildArgs := d.state.buildArgs.FilterAllowed(stateRunConfig.Env)

	saveCmd := cmdFromArgs									//以下判断能否复用本地缓存
	if len(buildArgs) > 0 {
	...
	runConfig.ArgsEscaped = true							//防止重复转义
	//create根据基础镜像ID以及运行容器时所需的runconfig信息，来创建Container对象进入create，调用Create，再调用ContainerCreate
	logrus.Debugf("[BUILDER] Command to be executed: %v", runConfig.Cmd)
	cID, err := d.builder.create(runConfig)					//create()函数执行创建Container对象操作
	if err != nil {
		return err
	}
	//Run运行Docker容器，其中包括创建容器文件系统、创建容器的命名空间进行资源隔离、为容器配置cgroups参数进行资源控制，还有运行用户指定的程序。
	if err := d.builder.containerManager.Run(d.builder.clientCtx, cID, d.builder.Stdout, d.builder.Stderr); err != nil {	//Run()函数执行运行容器操作
	...
	return d.builder.commitContainer(d.state, cID, runConfigForCacheProbe)		//commitContainer()函数执行提交新镜像操作。
}
