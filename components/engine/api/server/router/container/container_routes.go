...
func (s *containerRouter) postContainersStart(ctx context.Context, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	version := httputils.VersionFromContext(ctx)
	var hostConfig *container.HostConfig							//获取hostConfig配置信息
	// A non-nil json object is at least 7 characters.
	if r.ContentLength > 7 || r.ContentLength == -1 {				//非nil的json对象至少为7个字符。一系列检查
	...
	if err := httputils.ParseForm(r); err != nil {					//解析form表单
		return err
	}

	checkpoint := r.Form.Get("checkpoint")						//获得与键关联的值
...
func (s *containerRouter) postContainersCreate(ctx context.Context, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	if err := httputils.CheckForJSON(r); err != nil {			    //2>检查JSON文件
		return err
	}
